...
void eval_perf(unsigned int s, unsigned int E, unsigned int b) {
    ...
    for (i = 0; i < func_counter; i++) {
        ...
        printf(
            "\nFunction %d (%d total)\nStep 1: Validating and generating "
            "memory traces\n",
            i, func_counter);
        /* Use valgrind to generate the trace */

        sprintf(cmd,
                "valgrind --tool=lackey --trace-mem=yes --log-fd=1 -v "
                "./tracegen -M %d -N %d -F %d  > trace.tmp",
                M, N, i);
        flag = WEXITSTATUS(system(cmd));
        if (0 != flag) {
            printf(
                "Validation error at function %d! Run ./tracegen -M %d -N %d "
                "-F %d for details.\nSkipping performance evaluation for this "
                "function.\n",
                flag - 1, M, N, i);
            continue;
        }
        ...
        char cmd[255];
        sprintf(cmd, "./csim-ref -s %u -E %u -b %u -t trace.f%d > /dev/null", s,
                E, b, i);
        system(cmd);
