...
int Select(int n, fd_set *readfds, fd_set *writefds, fd_set *exceptfds, struct timeval *timeout) {
    ...
    if ((rc = select(n, readfds, writefds, exceptfds, timeout)) < 0) unix_error("Select error");
    return rc;
}
...
void *Mmap(void *addr, size_t len, int prot, int flags, int fd, off_t offset) {
    ...
    if ((ptr = mmap(addr, len, prot, flags, fd, offset)) == ((void *)-1)) unix_error("mmap error");
    return (ptr);
}
...
size_t Fread(void *ptr, size_t size, size_t nmemb, FILE *stream) {
    ...
    if (((n = fread(ptr, size, nmemb, stream)) < nmemb) && ferror(stream)) unix_error("Fread error");
    return n;
}
...
void Setsockopt(int s, int level, int optname, const void *optval, int optlen) {
    ...
    if ((rc = setsockopt(s, level, optname, optval, optlen)) < 0) unix_error("Setsockopt error");
}
...
void Connect(int sockfd, struct sockaddr *serv_addr, int addrlen) {
    ...
    if ((rc = connect(sockfd, serv_addr, addrlen)) < 0) unix_error("Connect error");
}
...
void Getnameinfo(const struct sockaddr *sa, socklen_t salen, char *host, size_t hostlen, char *serv, size_t servlen,
                 int flags) {
    int rc;

    if ((rc = getnameinfo(sa, salen, host, hostlen, serv, servlen, flags)) != 0) gai_error(rc, "Getnameinfo error");
}
...
int Open_clientfd(char *hostname, char *port) {
    ...
    if ((rc = open_clientfd(hostname, port)) < 0) unix_error("Open_clientfd error");
    return rc;
}
